...
@broker.task(task_name='send_login_email_code_task', retry_on_error=True, max_retries=10, delay=60)
@inject(patch_module=True)
async def send_login_email_code_task(
    command: SendLoginEmailCodeCommand,
    use_case: FromDishka[SendLoginEmailCodeUseCase],
) -> None:
    await use_case.execute(command=command)
