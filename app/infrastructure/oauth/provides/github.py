...
class GitHubOAuthProvider(IOAuthProvider):
    ...
    def provider_name(self) -> OAuthProvidersEnum:
        return OAuthProvidersEnum.GITHUB
    ...
    async def exchange_code(self, code: str) -> str:
        ...
        response = await self._http_client.post(url=token_url, headers=headers, data=request_body)
    ...
    async def get_user_data(self, token: str) -> OAuthProviderUserData:
        ...
        user_data: dict[str, str] = await self._http_client.get(url=user_data_url, headers=headers)

        provider_uid = user_data.get('id')
        ...
        if provider_email is None:
            ...
            user_emails = await self._http_client.get(url=user_email_url, headers=headers)
        ...
        provider_login = user_data['login']

        provider_name = user_data.get('name')
