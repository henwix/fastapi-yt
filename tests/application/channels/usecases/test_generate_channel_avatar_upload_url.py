...
from app.application.channels.usecases import GenerateChannelAvatarUploadUrlUseCase
...
async def test_generate_channel_avatar_upload_url_returns_correct_data(
    ...
    async with mock_container() as di:
        ...
        assert key.startswith(settings.s3_tmp_channel_avatars_key_prefix)
        assert key.endswith(expected_filename)
